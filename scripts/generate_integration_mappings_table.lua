...
local suazppuccin_path = vim.fn.getcwd() -- get to the root directory of the plugin
local lua_path = suazppuccin_path .. "/lua"

package.path = package.path .. string.format(";%s/?.lua", lua_path) .. string.format(";%s/?/init.lua", lua_path)

local integrations_path = lua_path .. "/suazppuccin/groups/integrations/"
local target_path = lua_path .. "/suazppuccin/utils/integration_mappings.lua"

---@return table mappings table holding plugin to suazppuccin name
local function create_table()
	local mappings = vim.iter(vim.fs.dir(integrations_path)):fold({}, function(m, filename, _)
		...
		local ok, mod = pcall(require, "suazppuccin.groups.integrations." .. filename)
		...
		local plugin_name = require("suazppuccin.lib.detect_integrations").parse_url(plugin_url)

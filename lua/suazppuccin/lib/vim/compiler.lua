local path_sep = require("suazppuccin").path_sep
...
function M.compile(flavour)
	local theme = require("suazppuccin.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[=[
...
set termguicolors
set background=%s
let g:colors_name = "suazppuccin-%s"]=],
	...
	if vim.g.suazppuccin_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/suazppuccin_error.lua"
		print(string.format(
			[[Catppuccin (error): Most likely some mistake made in your suazppuccin config

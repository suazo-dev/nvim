...
local C = require("suazppuccin.palettes").get_palette()
...
if ok then
	...
	if require("suazppuccin").flavour == "latte" then
		local latte = require("suazppuccin.palettes").get_palette "latte"
		sett.text = latte.base
		sett.bkg = latte.crust
	end

	if require("suazppuccin").options.transparent_background then sett.bkg = "NONE" end
